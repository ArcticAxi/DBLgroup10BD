...
intensity_slider_heatmap.onchange = function () {
...
radius_slider_heatmap.onchange = function () {
...
blur_slider_heatmap.onchange = function () {
...
// gets the min. timestamp for each user
// subtracts said timestamp from every timestamp
// flattens the array by losing user key and just pushing all values into another array
...
function heatmap(content, name, width, height, idName) {
    ...
    var canvasLayer = div.append('canvas').attr('id', 'canvas' + name).attr('class', 'heatmapCanvas').attr('width', width).attr('height', height);
